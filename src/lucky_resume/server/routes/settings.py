...
good hygiene. Writes persist to the user-config ``.env`` file resolved
by `lucky_resume.paths.env_file_path` (Windows: ``%APPDATA%\\lucky-resume``;
macOS: ``~/Library/Application Support``; Linux: ``~/.config``). The
file is auto-seeded from the bundled ``.env.example`` on first launch.

Writes invalidate the `get_settings` cache and take effect on the next
request.
"""
...
from lucky_resume.config import Settings, get_settings
from lucky_resume.paths import ensure_env_file, env_file_path
...
class SettingsPayload(BaseModel):
    ...
    # Surfaced so the frontend can display where settings actually live
    # and offer a Reveal button. Same path used by both reads and writes.
    env_file_path: str
...
def _settings_to_payload(s: Settings) -> SettingsPayload:
    ...
    data["env_file_path"] = str(env_file_path())
    return SettingsPayload(**data)


def _env_path() -> Path:
    """Resolve the runtime ``.env`` and create it (and its parent dir)
    on first access — keeps `set_key` from blowing up the very first
    time a user opens the Settings page on a clean install."""
