...
class Settings(BaseSettings):
    ...
    # `env_file` is resolved per-instance in `get_settings()` rather than
    # baked in here, so changes to `RESUME_OPERATOR_ENV_FILE` (or the cwd
    # in tests) are picked up after `cache_clear()` instead of being
    # frozen at module import time. Encoding is the only stable bit.
...
def get_settings() -> Settings:
    ...
    `_env_file` is passed at construction time so reads track whatever
    `paths.env_file_path()` currently resolves to. The Settings PUT
    route calls `cache_clear()` after writing, which forces the next
    `get_settings()` to re-resolve and re-read."""
